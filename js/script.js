...
/*    const skeleton = new THREE.SkeletonHelper(fox);
    ...
    // Définissez la pose initiale sur idlePose juste après le chargement
    fox.traverse((child) => {
...
let idleStartTime = Date.now(); // Suivre le début de la période d'inactivité
let transitionStartTime = null; // Initialiser comme nul, à définir après idlePose
const transitionDuration = 4000; // secondes pour la transition de pose
const idleDelay = 3000;          // seconds idle period

function interpolatePose() {
    const elapsed = Date.now() - transitionStartTime;
    const progressPhase1 = Math.min(elapsed / (transitionDuration / 2), 1); 
    const progressPhase2 = Math.min((elapsed - (transitionDuration / 2)) / (transitionDuration / 2), 1); 
    ...
    fox.traverse((child) => {
        if (child.name && idlePose.hasOwnProperty(`${child.name}_x`)) {
            ...
            if (progressPhase1 < 1) {
                ...
                // Phase 2 : Interpolation entre targetPose1 et targetPose
                child.rotation.set(
    ...
    if (progressPhase2 < 1) {
        requestAnimationFrame(interpolatePose); 
    }
}
